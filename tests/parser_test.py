...
class ParserTest(unittest.TestCase):
    ...
    def test_valid_var_declarations(self) -> None:
        ...
        assert(expr == Block(L,[VarDec(L,Identifier(L,'i'),Literal(L,23))],Literal(L,None)))
        assert(expr_2 == Block(L,[VarDec(L,Identifier(L,'c'),BinaryOp(L,Literal(L,5),'+',FunctionNode(L,Identifier(L,'f'),[Identifier(L,'x')])))],Literal(L,None)))
